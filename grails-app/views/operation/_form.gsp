...
<div class="fieldcontain ${hasErrors(bean: operationInstance, field: 'userGroup', 'error')} required">
	<label for="userGroup">
		<g:message code="operation.userGroup.label" default="User Group" />
		<span class="required-indicator">*</span>
	</label>
	<g:select id="userGroup" name="userGroup.id" from="${br.com.romalopes.andercidagastos.UserGroup.list()}" optionKey="id" required="" value="${operationInstance?.userGroup?.id}" class="many-to-one"/>
</div>
